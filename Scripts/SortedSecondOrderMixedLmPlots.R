...
for (i in 1:nrow(brackenT)) {
  ...
  pvals[i]<-anova(Model)[2,4]
}
adjPvals<-p.adjust(pvals, method = "BH")
plot_order<-order(pvals)
for (i in 1:nrow(brackenT)) {
  mainText<-paste0("Bracken(Species)\n", rownames(brackenT)[plot_order[i]], "\nANOVA Pval:", signif(pvals[plot_order[i]], 3), 
                   ", ", "adj. Pval=", signif(adjPvals[plot_order[i]], 3))
...
plot_order<-order(pvals)
for (i in 1:nrow(vsearchT)) {
  mainText<-paste0("vsearch\n", strsplit(rownames(vsearchT)[plot_order[i]], "\\|")[[1]][6],  "\nANOVA Pval:", signif(pvals[plot_order[i]], 3), 
                   ", ", "adj. Pval=", signif(adjPvals[plot_order[i]], 3))
