...
names<-paste0(metaData[colnames(brackenM), 5], "_",metaData[colnames(brackenM), 10])
colnames(brackenM)<-names
rownames(brackenM) <- make.names(names, unique = FALSE)

patient_ids<-unique(metaData$ID)

pdf("Plots/(days)BrayCurtisDistanceFromBaselineForEachPatient.pdf", width=20, height=15)
par(mfrow=c(3, 2))
par(mar=c(5, 6, 4, 1)+.1)

nonPreSamples<-metaData[which(metaData$bins != "PRE"), , drop = F]
minNeg<-abs(min(nonPreSamples$Timepoint))

plot(1, type = "n", xlim = range(log10(nonPreSamples$Timepoint+minNeg + 1)), ylim = range(brackenM),
     xlab = "log10(Days from PRE + 4)", ylab = "Bray-Curtis Distance", axes = FALSE)
axis(1)
axis(2)  # Add y-axis
box()
...
for (pid in patient_ids) {
  ...
  colnames(distance)<-log10(as.numeric(colnames(distance))+minNeg + 1)
...
for (pid in patient_ids) {
  ...
  plot(1, type = "n", xlim = range(log10(nonPreSamples$Timepoint+minNeg + 1)), ylim = range(brackenM),
       xlab = "log10(Days from PRE + 4)", ylab = "Bray-Curtis Distance", 
       main = pid, axes = FALSE)
  axis(1)
  axis(2)  # Add y-axis
  box()
