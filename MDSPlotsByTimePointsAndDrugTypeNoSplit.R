#MDS plots colored by timepoints based on MASTER_AMRlist_2023_03.csv Drug Type column but not breaking the column into sublists.
rm(list = ls())
library("dplyr")
library(stringr)
library("vegan")
...
drugType<-unique(amrMatchT$Drug.Class)
newamrT<-c()
for (i in 1:length(drugType)) {
  genes<-amrMatchT %>% filter(amrMatchT$Drug.Class == drugType[i])
...
drugType<-unique(rgiMatchT$Drug.Class)
newrgiT<-c()
for (i in 1:length(drugType)) {
  genes<-rgiMatchT %>% filter(rgiMatchT$Drug.Class == drugType[i])
...
drugType<-unique(vsearchMatchT$Drug.Class)
newvsearchT<-c()
for (i in 1:length(drugType)) {
  genes<-vsearchMatchT %>% filter(vsearchMatchT$Drug.Class == drugType[i])
...
pdf("Plots/MDSForDrugClassNoSplit(ColoredByTimePoints).pdf", width=18, height=6)
par(mfrow=c(1,3))
par(mar=c(5,6,4,1)+.1)
